...
async function getDataWithCache<T>(
	...
	try {
		...
		if (redis) {
			try {
				logger.debug(`Attempting to fetch ${key} from cache`, {
					context,
					requestId,
				});
				...
				if (cachedData) {
					try {
						...
						logger.warn(
							`Failed to parse cached data for ${key}, fetching fresh data`,
							{
								context,
								requestId,
								error:
			...
			logger.debug(`Redis client not available, skipping cache for ${key}`, {
				context,
				requestId,
			});
		}
		...
		logger.info(`Successfully fetched ${key} from database`, {
			context,
			requestId,
		});
...
export async function getProjects() {
	return getDataWithCache("projects", fallbackProjects, async () => {
		...
		if (!db) {
			logger.warn(
				"Database connection not available. Using fallback data for projects.",
				{
					context: "projects",
				},
			);
		...
		try {
			logger.debug("Attempting to fetch projects from database", {
				context: "projects",
			});
			...
			logger.debug(
				`Successfully fetched ${result.length} projects from database`,
				{
					context: "projects",
					data: result,
				},
...
export async function getProjectById(id: number) {
	...
	try {
		logger.debug(`Attempting to fetch project with id ${id} from database`, {
			context: "projects",
		});
		const query = db.select().from(projects).where(eq(projects.id, id));
		logger.debug("SQL query:", { context: "projects", query: query.toSQL() });
		const result = await query;
		logger.debug(`Successfully fetched project with id ${id} from database`, {
			context: "projects",
			data: result[0],
		});
		return result[0];
...
export async function getTestimonials() {
	return getDataWithCache("testimonials", fallbackTestimonials, async () => {
		...
		if (!db) {
			logger.warn(
				"Database connection not available. Using fallback data for testimonials.",
				{
		...
		try {
			logger.debug("Attempting to fetch testimonials from database", {
				context: "testimonials",
			});
			...
			const result = await query;
			logger.debug(
				`Successfully fetched ${result.length} testimonials from database`,
				{
					context: "testimonials",
					data: result,
				},
...
export async function getTestimonialById(id: number) {
	...
	try {
		logger.debug(
			`Attempting to fetch testimonial with id ${id} from database`,
			{ context: "testimonials" },
		);
		...
		const result = await query;
		logger.debug(
			`Successfully fetched testimonial with id ${id} from database`,
			{
				context: "testimonials",
				data: result[0],
			},
...
export async function createContactSubmission(
	data: Omit<typeof contactSubmissions.$inferInsert, "id" | "createdAt">,
) {
	const db = await getDb();
	if (!db) {
		logger.warn("Database connection not available.", {
	...
	try {
		logger.debug("Attempting to create contact submission in database", {
			context: "contactSubmissions",
		});
		...
		const result = await query;
		logger.debug(
			`Successfully created contact submission with id ${result[0].id} in database`,
			{
				context: "contactSubmissions",
				data: result[0],
			},
