...
async function initializeRedisClient() {
  try {
    if (!process.env.KV_URL) {
      throw new RedisConfigError("Redis environment variables are not properly configured")
    }

    logger.info("Initializing Redis client with Upstash credentials", { context: "redis" })

    redisClient = new Redis({
  url: 'https://famous-tetra-27230.upstash.io',
  token: 'AWpeAAIjcDFiZTRjZGRiNDU0Mjk0NGFlYjI2NzE0YTVkZDE2M2Q2N3AxMA',
})
